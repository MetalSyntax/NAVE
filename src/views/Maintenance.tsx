...
import { SlidersHorizontal, CalendarClock, PenTool, ImagePlus, Edit3, Trash2, X, Save, BellRing, CheckCircle2, Plus, ArrowLeft } from 'lucide-react';
...
export function MaintenanceScreen() {
  ...
  const latestLogs = useMemo(() => maintenanceLogs.slice(0, 5), [maintenanceLogs]);
      ...
      <div className="animate-in fade-in duration-500 pb-12">
        ...
        <div className="flex items-center gap-4 mb-6">
          <button
            ...
            className="w-10 h-10 rounded-full bg-surface-container flex items-center justify-center hover:bg-surface-high transition-colors text-on-surface animate-in fade-in"
          >
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      onChange={e => setEditingMaint({...editingMaint, cost: parseFloat(e.target.value) || 0})}
                      className={inputCls}
                      type="number"
                    />
                  </div>
  ...
  return (
    <div className="animate-in fade-in duration-500 pb-12">
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        {/* KM Restantes + Intervalo — tarjeta unificada */}
        <div className="sm:col-span-2 bg-surface-container rounded-xl p-6 shadow-elevation-1">
          <div className="flex items-stretch justify-between gap-6 h-full">
            <div className="flex flex-col justify-between flex-1">
              ...
              <div className="font-headline text-4xl font-black tracking-tight italic mt-4">
                {stats.remainingKm.toLocaleString()} <span className="text-base font-bold text-surface-variant">KM</span>
              </div>
            </div>
            <div className="w-px bg-outline-variant/30 flex-shrink-0" />
            <div className="flex flex-col justify-between flex-1">
              <div>
                <SlidersHorizontal className="text-primary mb-3 w-5 h-5" />
                <span className="font-label text-[9px] font-black tracking-widest uppercase mb-1 block text-tertiary">
                  {t('maintenance:interval_configured')}
                </span>
              </div>
              <div className="mt-4">
                <div className="font-headline text-2xl font-black tracking-tight">
                  {stats.interval.toLocaleString()} <span className="text-sm font-bold text-surface-variant">KM</span>
                </div>
                <div className="text-[10px] text-surface-variant font-bold uppercase tracking-wide mt-1">
                  Próximo: {stats.nextServiceKm.toLocaleString()} KM
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="bg-surface-container rounded-xl p-6 mb-6 shadow-elevation-1">
        <div className="flex flex-wrap justify-between items-center w-full mb-2">
          ...
          <button
            ...
            className={`mt-2 sm:mt-0 px-5 py-2 font-headline text-xs uppercase tracking-widest rounded-full transition-all ${
              showForm
          ...
          {maintenanceLogs.length > 0 && (
            <div className="flex justify-center pt-4">
              <button
                ...
                className="px-6 py-3 bg-surface-container hover:bg-surface-high text-primary hover:text-primary/95 font-headline text-xs font-bold uppercase tracking-widest rounded-full transition-all shadow-elevation-1"
              >
                Ver todos los servicios ({maintenanceLogs.length})
              </button>
            </div>
          )}
        </div>
      ...
      {/* MD3 Card — Service Schedules / Recordatorios (configuraciones al final) */}
      <div className="bg-surface-container rounded-xl p-6 mb-6 shadow-elevation-1">
            ...
            <table className="w-full text-left border-collapse min-w-[600px]">
              <thead>
                <tr className="border-b border-outline-variant/30">
                  <th className="pb-3 pt-2 font-label text-[10px] font-extrabold text-secondary tracking-[0.15em] uppercase">{t('maintenance:schedule_status')}</th>
                  ...
                  <th className="pb-3 pt-2 font-label text-[10px] font-extrabold text-secondary tracking-[0.15em] uppercase">{t('maintenance:schedule_next')}</th>
                ...
                {alerts.map(({ schedule, nextKm, remaining, level }) => {
                  ...
                  return (
                    <tr key={schedule.id} className="border-b border-outline-variant/10 hover:bg-surface-low/50 transition-colors">
                      <td className="py-4">
                        <span className={`px-2.5 py-1 rounded-full font-label text-[9px] font-extrabold uppercase tracking-widest whitespace-nowrap ${chip.cls}`}>
                          {chip.text}
                        </span>
                      </td>
                      <td className="p-4 font-headline font-bold uppercase text-on-surface">
